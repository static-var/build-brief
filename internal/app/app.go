...
func writeDoctorUsage(w io.Writer) {
	...
	fmt.Fprintf(w, "  --config PATH             Optional custom match config (%d rules; %d unique matches per rule)\n", config.CustomMatchRuleLimit, config.CustomMatchUniqueResultLimitPerRule)
	fmt.Fprintln(w, "  Relative --config and BUILD_BRIEF_CONFIG paths resolve from the effective")
	fmt.Fprintln(w, "  project directory: --project-dir, then BUILD_BRIEF_PROJECT_DIR, then the")
	fmt.Fprintln(w, "  current working directory. --project-dir takes precedence over")
	fmt.Fprintln(w, "  BUILD_BRIEF_PROJECT_DIR; absolute paths remain unchanged.")
